...
% fkine on qcobot-> Tcobot
figure('Name','fkine')
Mob_Cob.plot(qcobot,'workspace',[-1.5, 1.5, -1.5, 1.5,0,1.5],'scale',0.4)
%%
pause
Mob_Cob.plot(qcobot,'workspace',[-2, 2, -2, 2,0,2],'scale',0.4)
Tcobot=Mob_Cob.fkine(qcobot)
pause
close
% ikine on Tcobot
qcobot_out_ikcon=Mob_Cob.ikcon(Tcobot,q0);
figure('Name','ikine')
Mob_Cob.plot(qcobot_out_ikcon,'workspace',[-2, 2, -2, 2,0,2],'scale',0.4);
Tcobot_out_ikcon=Mob_Cob.fkine(qcobot_out_ikcon)

%% Jacobian function check
% jacob0 = J(theta) in global frame
% jacobe= J(theta) in ee frame 
J0_qcobot=Mob_Cob.jacob0(qcobot);
J0_q0=Mob_Cob.jacob0(q0);

Je_qcobot=Mob_Cob.jacobe(qcobot);
Je_q0=Mob_Cob.jacobe(q0);

% find Je with J0 for translational vel.
Je_qcobot(1:3,1:9);
inv(Ree_qcobot)*J0_qcobot(1:3,1:9);
...
figure(3)
Mob_Cob.plot(qtraj,'workspace',[-1.5, 1.5, -1.5, 1.5,0,1.5],'scale',0.4)
%%
ctraj(Tee_q0,Tee_qcobot,10)


%%

figure(2)
Mob_Cob.plot(out.simout.Data,'workspace',[-2000, 2000, -2000, 2000,0,2000],'fps',100)
% 
% Ts=ctraj(T0,T,length(t));
% % plot(t,Ts.transl);
% 
% qctraj=Mob_Cob.ikine(Ts,'mask',[1 1 1 0 0 1])
...
M=Mob_Cob.inertia(q)
C=Mob_Cob.coriolis(q,qd)
G=Mob_Cob.gravload(q)

Q=(M*qdd')'+(C*qd')'+G
Qfun=Mob_Cob.rne(q,qd,qdd)
